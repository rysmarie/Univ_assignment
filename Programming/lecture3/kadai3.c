...
double neuton(double (*func)(double), double (*deriv)(double), double x,
              double y) {
...
double deriv(double x) { return exp(x) + 1; }

double func2(double x) { return pow(x, 5); }
double deriv2(double x) { return 5 * pow(x, 4); }
int main() {
